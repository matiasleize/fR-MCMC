...
import torch
from utils import CustomCondition, f_R_reparams, _shape_manager
import yaml
...
nets = torch.load(f'{path}/nets_f_R.ph', map_location=torch.device('cpu'))
net_INT = torch.load(f'{path}/nets_f_R_d_L.ph', map_location=torch.device('cpu'))
...
# print(H(1, theta_example))

# plt.plot(z_examples, m_b_tilde(z_examples, z_examples, theta_example))
plt.plot(z_examples, H(z_examples, theta_example))
# plt.xscale('log')
plt.show()