...
def log_prior(theta):
    omega_m, b, H_0 = theta
    if (0.01 < omega_m < 0.8 and -40 < b < -1 and 50 < H_0 < 90):
        #if b > (-3 * omega_m + 8)/(omega_m-1):
        return 0.0
    return -np.inf
...
def log_prior_1(theta):
    omega_m, b, H_0 = theta
    if (0.001 < omega_m < 0.999 and -10 < b < 10 and 50 < H_0 < 80):
        #if b > 8 * omega_m/(omega_m-1):
            return 0.0
    return -np.inf
...
pos = sol + 1e-4 * np.random.randn(100, 3)
nwalkers, ndim = pos.shape
#%%
# Set up the backend
os.chdir(path_datos_global+'/Resultados_cadenas/')
filename = "sample_cron_omega_b_negativos_constrain_1_extra_H0.h5"
