...
with np.load('valores_medios_HS_cron+SN_3params_M_fijo.npz') as data:
    sol = data['sol']
print(sol)
...
log_likelihood = lambda theta: -0.5 * params_to_chi2(theta,params_fijos,
                zcmb, zhel, Cinv,mb,z_data, H_data, dH, M_fijo=True)
#%% Definimos las gunciones de prior y el posterior

def log_prior(theta):
    omega_m, b, H0 = theta
    if (0.05 < omega_m < 0.5 and 0 < b < 5 and 60 < H0 < 80):
        return 0.0
    return -np.inf
...
filename = "sample_HS_cron+SN_3params_M_fijo.h5"
