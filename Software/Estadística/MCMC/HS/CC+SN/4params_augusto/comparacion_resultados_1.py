...
samples_mati_20 = reader_mati.get_chain(flat=True,discard=burnin,thin=thin)[:,3]
...
#thin = int(0.5 * np.min(tau))
samples_agus = reader_agus.get_chain(flat=True,discard=burnin,thin=thin)[:,2]
...
thin = int(0.5 * np.min(tau))
samples_mati_12 = reader_mati_1.get_chain(flat=True,discard=burnin,thin=thin)[:,3]
