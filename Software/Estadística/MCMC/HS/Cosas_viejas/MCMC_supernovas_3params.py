...
#np.random.seed(42)
log_likelihood = lambda theta: -0.5 * params_to_chi2_H0_fijo(ci, theta,
                                [H_0,n], zcmb, zhel, Cinv, mb)

#%% Definimos las funciones de prior y el posterior

def log_prior(theta):
    M,omega_m,b = theta
    if -22 < M < -19 and 0.01 < omega_m < 0.9  and -10 < b < 10  :
        return 0.0
    return -np.inf
...
filename = "sample_supernovas_M_omega_b_101.h5"
