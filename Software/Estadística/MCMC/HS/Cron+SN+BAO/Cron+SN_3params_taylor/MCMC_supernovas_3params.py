...
with np.load('valores_medios_HS_cron+SN+BAO_3params_taylor.npz') as data:
    sol = data['sol']
print(sol)
...
log_likelihood = lambda theta: -0.5 * params_to_chi2_taylor(theta,params_fijos,zcmb, zhel, Cinv,mb,z_data, H_data, dH, dataset)
#%% Definimos las gunciones de prior y el posterior

def log_prior(theta):
    M, omega_m, b = theta
    if (-22 < M < -18 and  0.01 < omega_m < 0.5 and -3 < b < 3):
        return 0.0
    return -np.inf
...
filename = "sample_HS_cron+SN+BAO_3params_taylor.h5"
