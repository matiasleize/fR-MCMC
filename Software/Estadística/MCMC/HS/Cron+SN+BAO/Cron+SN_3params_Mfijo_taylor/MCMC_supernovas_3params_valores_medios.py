...
omega_m_true = 0.28
b_true = -0.54
H0_true =  67 #Unidades de (km/seg)/Mpc
n = 1
...
nll = lambda theta: params_to_chi2_taylor(theta,params_fijos,zcmb, zhel,
                    Cinv,mb,z_data, H_data,
                    dH, dataset, M_fijo=True,chi_riess=False)

initial = np.array([omega_m_true,b_true,H0_true])
soln = minimize(nll, initial, options = {'eps': 0.01}, bounds =((0.1,0.5),(-3, 3),(60,80)))
omega_m_ml, b_ml, H0_ml = soln.x

print(omega_m_ml,b_ml,H0_ml)

os.chdir(path_git + '/Software/Estadística/Resultados_simulaciones')
#np.savez('valores_medios_HS_cron+SN+BAO_3params_Mfijo_taylor', sol=soln.x)
np.savez('valores_medios_HS_cron+SN+BAO_3params_Mfijo_sin_riess_taylor', sol=soln.x)
