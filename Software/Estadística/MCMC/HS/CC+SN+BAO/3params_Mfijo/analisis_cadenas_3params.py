...
with np.load('valores_medios_HS_CC+SN+BAO_3params_Mfijo_taylor.npz') as data:
    sol = data['sol']
#%%
os.chdir(path_datos_global+'/Resultados_cadenas')
filename = "sample_HS_CC+SN+BAO_3params_Mfijo_taylor.h5"
...
graficar_cadenas(reader,
                labels = ['$\Omega_{m}$','b','$H_{0}$'],title='SN+BAO+CC HS (Taylor)')
#%%
burnin=30
burnin = int(2 * np.max(tau))
thin = int(0.5 * np.min(tau))
