...
log_likelihood = lambda theta: -0.5 * params_to_chi2(theta, [], zcmb,
                                                     zhel, Cinv, mb)
...
def log_prior(theta):
    M, omega_m, H0 = theta
    if (-20 < M < -18.5 and  0 < omega_m < 1 and 50 < H0 < 90):
        return 0.0
    return -np.inf
...
pos = sol + 1e-4 * np.random.randn(100, 3) #Defino la cantidad de caminantes.
