...
nll = lambda theta: params_to_chi2(theta,params_fijos,zcmb, zhel, Cinv,
                    mb,z_data, H_data, dH, M_fijo=True,chi_riess=False)

initial = np.array([omega_m_true,b_true,H0_true])
soln = minimize(nll, initial, options = {'eps': 0.01}, bounds =((0.1,0.5),(0, 4),(60,80)))
omega_m_ml, b_ml, H0_ml = soln.x

print(omega_m_ml,b_ml,H0_ml)

os.chdir(path_git + '/Software/Estadística/Resultados_simulaciones')
np.savez('valores_medios_HS_CC+SN_3params_M_fijo', sol=soln.x)

soln.fun/(len(z_data)+len(zcmb)+1-3) #0.9688667684641672
