...
with np.load('valores_medios_LCDM_AGN_1params_v2.npz') as data:
    sol = data['sol']
print(sol)
#Parametros fijos
H0 = 73.48
...
log_likelihood = lambda theta: -0.5 * params_to_chi2(theta, params_fijos,
                                            ...
                                            model='LCDM',errores_agrandados=True)
...
pos = sol + 1e-4 * np.random.randn(12, 1)

MCMC_sampler(log_probability,pos,
            filename = "sample_LCDM_AGN_1params_v2.h5",
            witness_file = 'witness_11_v2.txt',
            witness_freq = 5,
            max_samples = 100000)
