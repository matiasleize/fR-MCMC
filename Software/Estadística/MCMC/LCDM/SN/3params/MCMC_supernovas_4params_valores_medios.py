...
soln = minimize(nll, initial, options = {'eps': 0.01}, bounds =((-25,-18),(0.1,0.5),(62,75)))
...
np.savez('valores_medios_LCDM_SN_3params.npz', sol=soln.x)

num_data_CC = len(ds_CC[0])
num_data_SN = len(ds_SN[0])
# num_data_BAO = 20
#num_data_AGN = len(ds_AGN[0])
datos_totales = num_data_SN

soln.fun/(datos_totales-len(soln.x))
#%%
os.chdir(path_git+'/Software/Estadística/Resultados_simulaciones/')
with np.load('valores_medios_LCDM_SN_3params.npz') as data:
    sol = data['sol']
