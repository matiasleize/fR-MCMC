...
params_fijos = [H0_true,n,beta,n_agn]
...
initial = np.array([omega_m_true,b_true,l_true])
soln = minimize(nll, initial, options = {'eps': 0.01},
                bounds =((0.1,0.7),(0,0.5),(1,40)))
omega_m_ml,b_ml,l_ml = soln.x

print(omega_m_ml,b_ml,l_ml)

os.chdir(path_git + '/Software/Estadística/Resultados_simulaciones')
np.savez('valores_medios_HS_AGN_3params', sol=soln.x)
soln.fun / (len(Theta_data)-3)
#%%
os.chdir(path_git+'/Software/Estadística/Resultados_simulaciones/')
with np.load('valores_medios_HS_AGN_3params.npz') as data:
    sol = data['sol']
