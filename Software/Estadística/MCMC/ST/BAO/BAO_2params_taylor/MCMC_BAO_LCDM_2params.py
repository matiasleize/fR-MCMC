...
with np.load('valores_medios_ST_BAO_2params_taylor.npz') as data:
    sol = data['sol']
sol=[0.27,-0.1]
print(sol)
...
log_likelihood = lambda theta: -0.5 * params_to_chi2_taylor(theta,[H_0,n],
                                        dataset,model='ST')

#%% Definimos las funciones de prior y el posterior
def log_prior(theta):
    omega_m, b = theta
    if (0.1 < omega_m < 0.45 and -2 < b < 4):
        return 0.0
    return -np.inf
...
filename = "sample_ST_BAO_2params_taylor.h5"
